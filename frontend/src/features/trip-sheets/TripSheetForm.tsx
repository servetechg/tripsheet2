    ...
    <div style={{ ...page() }}>
      ...
      <div style={{ padding:"16px 14px 100px",maxWidth:700,margin:"0 auto" }}>
        <Card>
          ...
          <G2 cols={2}><Inp label="Truck Unit No." value={hdr.truckNo} onChange={e=>updH("truckNo",e.target.value)} placeholder="e.g. 32054"/><Inp label="Start Date" type="text" value={hdr.startDate} onChange={e=>updH("startDate",e.target.value)} placeholder="e.g. 4 May 2026"/></G2>
          <G2 cols={2}><Inp label="End Date" type="text" value={hdr.endDate} onChange={e=>updH("endDate",e.target.value)} placeholder="e.g. 12 May 2026"/><Inp label="Driver Name 1" value={hdr.driver1} onChange={e=>updH("driver1",e.target.value)}/></G2>
        ...
        <Card>
          ...
          {trips.map((t,i)=>(
            <div key={t.id} style={{ background:G.card2,border:`1px solid ${G.border2}`,borderRadius:10,padding:14,marginBottom:12 }}>
              ...
              <G2 cols={2}><Inp label="Pickup Date" type="text" value={t.pickupDate} onChange={e=>updT(t.id,"pickupDate",e.target.value)} placeholder="e.g. 4 May"/><Inp label="Drop Date" type="text" value={t.dropDate} onChange={e=>updT(t.id,"dropDate",e.target.value)} placeholder="e.g. 7 May"/></G2>

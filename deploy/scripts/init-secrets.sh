#!/usr/bin/env bash
# Write split secret files from prompts / generated values.
# Usage on VPS: ./deploy/scripts/init-secrets.sh
set -euo pipefail

SECRETS_DIR="${SECRETS_DIR:-/opt/tripsheet/secrets}"
mkdir -p "${SECRETS_DIR}"
chmod 700 "${SECRETS_DIR}"

read -r -p "VPS public IPv4: " VPS_IP
read -r -p "ACME email for HTTPS: " ACME_EMAIL
read -r -p "GitHub org/repo for GHCR (e.g. myorg/tripsheet) or 'tripsheet' for local: " IMAGE_REGISTRY_INPUT

DOMAIN="tripsheet.${VPS_IP}.sslip.io"
STAGING_DOMAIN="staging.tripsheet.${VPS_IP}.sslip.io"
POSTGRES_PASSWORD="$(openssl rand -hex 32)"
REDIS_PASSWORD="$(openssl rand -hex 32)"
JWT_SECRET="$(openssl rand -base64 48)"
JWT_SECRET_STAGING="$(openssl rand -base64 48)"
INTERNAL_API_KEY="$(openssl rand -hex 32)"
INTERNAL_API_KEY_STAGING="$(openssl rand -hex 32)"
PLATFORM_SECRETS_KEY="$(openssl rand -base64 48)"
PLATFORM_SECRETS_KEY_STAGING="$(openssl rand -base64 48)"
IMAGE_REGISTRY="${IMAGE_REGISTRY_INPUT:-tripsheet}"

cat > "${SECRETS_DIR}/infra.env" <<EOF
POSTGRES_USER=tripsheet
POSTGRES_PASSWORD=${POSTGRES_PASSWORD}
REDIS_PASSWORD=${REDIS_PASSWORD}
EOF

cat > "${SECRETS_DIR}/app.env" <<EOF
POSTGRES_USER=tripsheet
POSTGRES_PASSWORD=${POSTGRES_PASSWORD}
REDIS_PASSWORD=${REDIS_PASSWORD}
JWT_SECRET=${JWT_SECRET}
JWT_EXPIRES_IN=7d
INTERNAL_API_KEY=${INTERNAL_API_KEY}
PLATFORM_SECRETS_KEY=${PLATFORM_SECRETS_KEY}
TENANT_DEFAULT_ROUTING_MODE=tenant
TENANT_RUNTIME_MODE=enforce
TENANT_CACHE_TTL_MS=60000
CORS_ORIGIN=https://${DOMAIN}
APP_PUBLIC_ORIGIN=https://${DOMAIN}
INVITE_PUBLIC_ORIGIN=https://${DOMAIN}
CLOUDINARY_CLOUD_NAME=
CLOUDINARY_API_KEY=
CLOUDINARY_API_SECRET=
CLOUDINARY_FOLDER=tripsheet/documents
TWILIO_ACCOUNT_SID=
TWILIO_AUTH_TOKEN=
TWILIO_FROM_NUMBER=
SMTP_HOST=
SMTP_PORT=587
SMTP_SECURE=false
SMTP_USER=
SMTP_PASS=
SMTP_FROM=noreply@${DOMAIN}
IMAGE_REGISTRY=${IMAGE_REGISTRY}
IMAGE_TAG=local
COLOR=blue
EOF

cat > "${SECRETS_DIR}/staging.app.env" <<EOF
POSTGRES_USER=tripsheet
POSTGRES_PASSWORD=${POSTGRES_PASSWORD}
REDIS_PASSWORD=${REDIS_PASSWORD}
JWT_SECRET=${JWT_SECRET_STAGING}
JWT_EXPIRES_IN=7d
INTERNAL_API_KEY=${INTERNAL_API_KEY_STAGING}
PLATFORM_SECRETS_KEY=${PLATFORM_SECRETS_KEY_STAGING}
TENANT_DEFAULT_ROUTING_MODE=tenant
TENANT_RUNTIME_MODE=enforce
TENANT_CACHE_TTL_MS=60000
CORS_ORIGIN=https://${STAGING_DOMAIN}
APP_PUBLIC_ORIGIN=https://${STAGING_DOMAIN}
INVITE_PUBLIC_ORIGIN=https://${STAGING_DOMAIN}
CLOUDINARY_CLOUD_NAME=
CLOUDINARY_API_KEY=
CLOUDINARY_API_SECRET=
CLOUDINARY_FOLDER=tripsheet/staging/documents
TWILIO_ACCOUNT_SID=
TWILIO_AUTH_TOKEN=
TWILIO_FROM_NUMBER=
SMTP_HOST=
SMTP_PORT=587
SMTP_SECURE=false
SMTP_USER=
SMTP_PASS=
SMTP_FROM=noreply@${STAGING_DOMAIN}
IMAGE_REGISTRY=${IMAGE_REGISTRY}
IMAGE_TAG=local
EOF

cat > "${SECRETS_DIR}/edge.env" <<EOF
DOMAIN=${DOMAIN}
STAGING_DOMAIN=${STAGING_DOMAIN}
ACTIVE_COLOR=blue
CADDY_ACME_EMAIL=${ACME_EMAIL}
EOF

chmod 600 "${SECRETS_DIR}/"*.env
echo "Wrote secrets to ${SECRETS_DIR}"
echo "Production: https://${DOMAIN}"
echo "Staging:    https://${STAGING_DOMAIN}"
echo "Verify DNS: dig +short ${DOMAIN} && dig +short ${STAGING_DOMAIN}"
